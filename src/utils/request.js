...
export default function request(url, option) {
  ...
  if (
    ...
    if (newOptions.body instanceof ReadableStream) {
      newOptions.headers = {
  ...
  return (
    fetch(url, newOptions)
      ...
      .then(response => {
        ...
        if (newOptions.responseType === 'arraybuffer') {
          return response;
        }
        return response.json();
      })
      ...
      .catch(e => {
        message.error(`${e}`);
        message.error(`接口错误，请找开发人员`);
        notification.error({
          message: `请求错误 ${e}: ${response.url}`,
          description: e,
        });
        const status = e.name;
        if (status === 401) {
